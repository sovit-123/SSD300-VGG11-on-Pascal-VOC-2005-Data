...
def calculate_mAP(det_boxes, det_labels, det_scores, true_boxes, true_labels, true_difficulties):
    ...
    :param det_boxes: list of tensors, one tensor for each image containing
                      detected objects' bounding boxes
    :param det_labels: list of tensors, one tensor for each image containing
                       detected objects' labels
    :param det_scores: list of tensors, one tensor for each image containing
                       detected objects' labels' scores
    :param true_boxes: list of tensors, one tensor for each image containing
                       actual objects' bounding boxes
    :param true_labels: list of tensors, one tensor for each image containing
                        actual objects' labels
...
def xy_to_cxcy(xy):
    ...
    :return: bounding boxes in center-size coordinates, 
             a tensor of size (n_boxes, 4)
    """
...
def cxcy_to_xy(cxcy):
    ...
    :param cxcy: bounding boxes in center-size coordinates, 
                 a tensor of size (n_boxes, 4)
    :return: bounding boxes in boundary coordinates, 
             a tensor of size (n_boxes, 4)
    """
...
def cxcy_to_gcxgcy(cxcy, priors_cxcy):
    """
    Encode bounding boxes (that are in center-size form) w.r.t. the 
    corresponding prior boxes (that are in center-size form).
    ...
    In the model, we predict bounding box coordinates in this encoded form.

    :param cxcy: bounding boxes in center-size coordinates, 
                 a tensor of size (n_priors, 4)
    :param priors_cxcy: prior boxes with respect to which we perform encoding,
                        a tensor of size (n_priors, 4)
    :return: encoded bounding boxes, a tensor of size (n_priors, 4)
    """

    # the 10 and 5 below are referred to as 'variances' in the original...
    # ...Caffe repo, completely empirical
    # they are for some sort of numerical conditioning, for...
    # ...'scaling the localization gradient'
...
def gcxgcy_to_cxcy(gcxgcy, priors_cxcy):
    ...
    :param gcxgcy: encoded bounding boxes, i.e. output of the model, 
                   a tensor of size (n_priors, 4)
    :param priors_cxcy: prior boxes with respect to which the encoding is 
                        defined, a tensor of size (n_priors, 4)
    :return: decoded bounding boxes in center-size form, 
             a tensor of size (n_priors, 4)
    """
...
def save_checkpoint(epoch, model, optimizer):
    ...
    state = {'epoch': epoch,
             'model': model,
             'optimizer': optimizer}
    filename = '../model_checkpoints/checkpoint_ssd300_vgg11.pth.tar'
